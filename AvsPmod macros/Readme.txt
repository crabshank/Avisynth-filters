...
Insert new braces WP - curr - focus.py - Adds brace and changes previous brace depending on the position of the last bookmark, and sets the latest brace to the RGB valuse that the cursor was over when the macro was triggered. Also checks the script for 'focusser' and 'focus_frame' (for use with De(focus).py and focusOnFrame {function})
